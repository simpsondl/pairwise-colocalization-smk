...
while IFS= read -r line; do
	# Extract columns
	gwas=$(echo "$line" | awk '{print $2}')
	gwasb=$(echo "$line" | awk '{print $3}')
	...
	awk -v awk_chr="$chr" -v awk_start="$start" -v awk_end="$end" '
        ...
        ' ${gwas_fname} > tmp_${job_id}.txt
	sort -k2,2 tmp_${job_id}.txt | cut -f2,5,6 > tmp_${job_id}_sorted.txt
	echo `wc -l tmp_${job_id}_sorted.txt`
	...
	awk -v awk_chr="$chr" -v awk_start="$start" -v awk_end="$end" '
        ...
        ' ${gwasb_fname} > tmp_${job_id}.txt
        sort -k2,2 tmp_${job_id}.txt | cut -f2,5,6 > tmp_${job_id}_sortedb.txt
        echo `wc -l tmp_${job_id}_sortedb.txt`

	# merge stats across studies
	join -1 1 -2 1 tmp_${job_id}_sorted.txt tmp_${job_id}_sortedb.txt > ${output_fpath}2 && mv ${output_fpath}2 ${output_fpath}
...
# Clean up
rm tmp_${job_id}.txt
rm tmp_${job_id}_sorted.txt
rm tmp_${job_id}_sortedb.txt
